...
metro_venue_map = 
  ...
  scale_colour_manual(values = c(bv.navy, bv.purple), guide = "none") +
  scale_fill_manual(values = c(bv.white, bv.crystal), guide = "none") +
...
ggplot() +
  ...
  scale_x_date(
    name = NULL,
    date_labels = "%Y",
    breaks = seq.Date(from = ymd("2023-6-1"), 
                      to = ymd("2026-6-1"), 
                      by =   "years"),
    expand = c(0,0)
  ) +
  coord_cartesian(xlim = ymd("2023-8-1","2026-7-1"), ylim = c(0,16e6)) +
  scale_colour_dffh(palette = "extended", name = NULL, guide = "none")+
  theme_vps_djcs(base_size = 10)+
...
regional_lga_data =
  vcglR::egm_lga_data |> 
  filter(str_detect(lga_name,"Ballarat|Bendigo|Latrobe|Wodon")) |> 
  filter(str_detect(measure_type, "Exp"), !is.na(value)) 
...
ggplot() +
  ...
  coord_cartesian(xlim = ymd("2023-8-1","2026-7-1"), ylim = c(0,8e6)) +
  scale_colour_djcs(palette = "primary", name = NULL, guide = "none")+
  theme_vps_djcs(base_size = 10)+
  theme(legend.position = "right")
```
...
sa3_plot = 
  ...
  theme_vps_djcs(base_size = 10) +
  labs(caption = "Note: This chart uses (SA3) geographical areas.")
